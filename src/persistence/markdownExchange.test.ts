...
import {
  ...
  importResumeFromHtmlContent,
  importResumeFromMarkdown,
} from '@/persistence/markdownExchange'

describe('markdown exchange', () => {
  it('exports structured resume as markdown with relative avatar path', () => {
    const resume = createEmptyResume('测试简历')
    resume.basics.name = '李四'
    resume.basics.summary = '熟悉 **Java** 与 Redis'
    resume.sectionOrder = [
      resume.sections[2].id,
      resume.sections[1].id,
      resume.sections[0].id,
      resume.sections[3].id,
      resume.sections[4].id,
    ]
    ...
    const markdown = exportResumeToMarkdown(resume, {
      photoMarkdownPath: 'images/avatar.png',
    })

    expect(markdown).toContain('# 李四')
    expect(markdown).toContain('![头像](images/avatar.png)')
  ...
  it('exports html preview as markdown and strips non-avatar images', () => {
    const resume = createEmptyResume('测试')
    const markdown = exportResumeToMarkdown(resume, {
      photoMarkdownPath: 'images/avatar.jpg',
      preferredHtml: `
        <!doctype html>
        <html>
          <head>
            <style>.hero{color:#0f766e;}</style>
          </head>
          <body>
            <h1>袁帅</h1>
            <img src="data:image/png;base64,template-photo" alt="模板图片" />
            <p class="hero"><strong>求职意向：</strong>后端开发实习生</p>
            <h2>项目经历</h2>
...
`
    ...
    expect(workSection?.items[0]?.title).toBe('云材科技')
...
<div>
  ...
  <h2>教育背景</h2>
  <div>
    <span>荆楚理工学院 路 计算机科学与技术 路 本科 路 GPA: 3.6</span>
...
`
  ...
  it('can import html content without preserving source-html mode', () => {
    const resume = importResumeFromHtmlContent(
      `
        <div>
          <h1>袁帅</h1>
          <p>18086169268 | yuanshuai1480@gmail.com</p>
          <h2>教育背景</h2>
          <div><span>荆楚理工学院 | 计算机科学与技术</span><span>2023-09 - 2027-06</span></div>
        </div>
      `,
      [],
    ...
    expect(resume.sourceFormat).toBe('structured')
    expect(resume.previewMode).toBe('structured')
    expect(resume.rawSourceHtml).toBeNull()
    expect(resume.basics.name).toBe('袁帅')
  })
})
